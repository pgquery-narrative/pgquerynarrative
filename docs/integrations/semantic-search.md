...
The root Compose file builds its Postgres image from `POSTGRES_IMAGE` plus HypoPG,
so switch the base to a pgvector image and rebuild:

```bash
# Base image includes pgvector binaries
POSTGRES_IMAGE=pgvector/pgvector:pg16 docker compose build postgres
...
Set `EMBEDDING_BASE_URL` and `EMBEDDING_MODEL` (see [Configuration – Embeddings](../reference/configuration.md#embeddings)).
