...
`main` currently has **no effective protection**. Checked with the GitHub API on
2026-09-10:

- Classic branch protection on `main`: none (`GET …/branches/main/protection` → 404).
- One active repository ruleset, "Branch rule", with deletion, non-fast-forward, code
  scanning and code-quality rules — but its `ref_name.include` list is **empty**, so it
  targets no branch at all, and it requires neither a pull request nor any status check.

Every check described below runs on pull requests, but nothing enforces that they passed
before a merge — protection is the piece that turns a green CI into an actual gate. That
includes the documentation contract: `Docs` runs `mkdocs build --strict` and
`make docs-contract-check`, and it only stops drift once it is required.
