...
[Demo dataset](dataset.md)). Measured **2026-06-08**.
...
filtered columns (see [REST API examples](../integrations/rest-api.md) and
[Understand plan findings](../workflows/plan-findings.md)).

!!! note "What PgQueryNarrative did and did not do here"
    PgQueryNarrative produced the plan findings and flagged the missing index
    shape. The `CREATE INDEX` below was applied by a person in `psql`:
    PgQueryNarrative never applies a proposed rewrite, index or DDL to the
    analytical database. The timings are `EXPLAIN (ANALYZE, BUFFERS)` runs on one
    machine, so read them as observed samples, not a benchmark.
...
Requires the large seed ([Demo dataset](dataset.md)):
...
curl -s -X POST http://localhost:8080/api/v1/queries/explain \
  ...
  | jq '{evidence_mode, server_execution_time_ms, planning_time_ms, seq_scans: ([.findings[] | select(.is_seq_scan)] | length)}'
```

`"analyze": true` executes the query and is refused unless the server runs with
`SECURITY_EXPLAIN_ANALYZE_ENABLED=true` (the local Compose stack sets it; production
StrictMode forbids it). `evidence_mode` is `observed` only when PostgreSQL reported an
execution time; without ANALYZE it is `estimated` and `server_execution_time_ms` is absent.
Field meanings: [Evidence and status vocabulary](../reference/evidence.md#timing-fields).
...
**See also:** [Demo dataset](dataset.md) · [REST API examples](../integrations/rest-api.md) · [Partition migration](https://github.com/pgquery-narrative/pgquerynarrative/blob/main/app/db/migrations/000018_partition_demo_sales.up.sql)
