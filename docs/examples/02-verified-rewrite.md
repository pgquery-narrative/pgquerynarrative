# Verifying a rewrite, not just proposing one: a bug that surfaced along the way
...
That's a **single aggregate pass per side**, no sort, no row transfer beyond
...
(Quoted verbatim from the running app, with the finding text's own punctuation
normalized to match this document's style; the DOM structure shown, plain
`<p>`, not `<pre>`, is unchanged.) And the SQL export, regenerated after the fix:
